...
BANNED = {
    "pad": {
        ...
        "疗效夸大": ["疗效", "治疗"],  # 注：法定警示语"本品不是药品，不具有治疗、护理、保健作用"是必标项，扫描命中该句属正常语境，人工复核时排除
    },
    "wetwipe": {
...
GENERIC_WORDS = {
    ...
    # 无评价方法的功能造词（2026-09-25 轮次新增）：GB 15979-2024 / GB/T 8939 /
    # GB/T 28004 均不给这些功能设限、也无测定方法，属无依据宣称。
    # 真实样本：2026-07 苏州绿叶"远红外离子纳米银磁动力"卫生巾搭车《黄帝内经》；
    # 2025-07 "棉密码"卫生巾硫脲事件（均无官方处罚结论）。
    "无依据功能造词": ["磁动力", "磁能量", "远红外", "纳米银", "负离子", "量子",
                   "石墨烯", "基因芯片", "活血", "暖宫", "驱寒", "延年"],
}
...
# 法定警示语模板：GB 38598 抗（抑）菌制剂、卫生湿巾等品类的**必须标注**句。
# 命中"治疗"等词出现在这句里 = 合规行为而非宣称，跳过。
LEGAL_WARNING_TEMPLATES = (
            ...
            for w in words:
                ...
                while True:
                    ...
                    if in_legal_context(line, idx, w):
                        start = idx + len(w)
                        continue
                    # 法定警示语模板（"本品不是药品，不具有治疗…"）跳过
                    if any(t in line for t in LEGAL_WARNING_TEMPLATES) and w in (
                        "治疗", "疗效", "护理", "保健"):
                        start = idx + len(w)
                        continue
